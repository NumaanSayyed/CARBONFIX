  ...
  const fetchProofs = async () => {
    try {
      const response = await axios.get(
        `https://carbonfix-backend-5y3e.onrender.com/serviceProviders/proofs/${projectId}`,
        {
          headers: {
  ...
  const handleSendToAdmin = async (proofId: string) => {
    try {
      ...
      const response = await axios.post(
        `https://carbonfix-backend-5y3e.onrender.com/serviceProviders/proofs/${proofId}/send-to-admin`, // Your backend API URL
